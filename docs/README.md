...
- [`A8_EXECUTION_GUIDED_STATE_SEARCH.md`](A8_EXECUTION_GUIDED_STATE_SEARCH.md) — current primary A8 proposal: treat the executor as a deterministic electronic-state transition model, preserve multiple executable states under bounded search, and add a learned goal-reachability value only after a no-training search gate.
