...
|---|---|---|
| **Editor or reviewer** | `README` → `SCIENTIFIC_THESIS` → `PROOF_CENTRIC_EXPERIMENT_PLAN` | Understand the scientific gap, claim ladder, and evidence required |
...
| **Data/evidence curator** | `KNOWLEDGE_ABLATIONS` → `TEXTBOOK_RAG` → `MECHANISTIC_PRIMITIVE_LIBRARY` | Build frozen evidence conditions with provenance and controls |
...
8. [`TEXTBOOK_RAG.md`](TEXTBOOK_RAG.md) — corpus provenance, bounded retrieval, and evidence-card contract.
...
5. Evaluate H2 on a non-empty, composition-disjoint split whose primitives all occur in train.
6. Evaluate H3 with frozen evidence, irrelevant context, direct open-book, and evidence-content interventions.
7. Scale models or add RL only after pilot gates pass.
8. Treat planning as a downstream extension.
...
- [`../knowledge/README.md`](../knowledge/README.md) — source licensing and asset policy.
...
`tests/test_documentation_contract.py` protects the semantic contract rather than exact prose. It checks that documentation:
