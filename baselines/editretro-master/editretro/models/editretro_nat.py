    ...
    def forward_decoder_reposition(self,
        ...
        if can_reposition_word.sum() != 0:
            ...
            if repos_beam > 1:
                ...
                rank2[c] = ranki.long().to(rank1.device)[c]
    ...
    def forward_decoder_mask(self,
        ...
        if can_ins_mask.sum() != 0:
            ...
            # TODO: length beam
                ...
                rank2[c] = mask_ins_pred_reshape.long().to(rank1.device)[c]
    ...
    def forward_decoder_token(self,
        ...
        if can_ins_word.sum() != 0:
            ...
            # TODO: top-k decoding
                ...
                rank2[c] = word_ins_pred_reshape.long().to(rank1.device)[c]
                score2[c] = word_ins_score_reshape.float().to(score1.device)[c]
            ...
            # A very early checkpoint can predict zero mask insertions for an
            # entire batch. Keep the fixed EditRetro beam cardinality even
            # though there are no token alternatives to branch on.
            output_tokens = output_tokens.repeat_interleave(token_beam, dim=0)
            output_marks = output_marks.repeat_interleave(token_beam, dim=0)
            output_scores = output_scores.repeat_interleave(token_beam, dim=0)
            if attn is not None:
