...
def load_libnat():
    global _LIBNAT_CACHE
    if _LIBNAT_CACHE is not None:
        return _LIBNAT_CACHE
    try:
        from fairseq import libnat_cuda
        _LIBNAT_CACHE = (libnat_cuda, True)
        return _LIBNAT_CACHE
...
def _get_advanced_reposition_targets(in_tokens, out_tokens, padding_idx):
    ...
    def _get_reposition_targets_cpu(in_tokens, out_tokens, padding_idx):
        ...
        word_reposition_targets = [
            labels[:in_seq_len] + [0 for _ in range(max(0, in_seq_len - len(labels)))]
