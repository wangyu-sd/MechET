...
def _label_tuple(label_type, label_idx, label_template):
    # Persist plain Python ints so CSV round-trips do not require numpy in eval.
    return (str(label_type), int(label_idx), int(label_template))
            ...
            try:
                ...
                if args['retro']:
                    ...
                    for edit_type, edit in edits.items():
                        for e in edit:
                            if edit_type in ['A', 'R']:
                                rxn_labels.append(_label_tuple('a', atom_sites.index(e), template_dicts['atom'][template_H]))
                            else:
                                rxn_labels.append(_label_tuple('b', bond_sites.index(e), template_dicts['bond'][template_H]))
                    reactants.append(reactant)
                    products.append(product)
                    reagents.append(reagent)       
                    labels.append(rxn_labels)
                    ...
                    for edit_type, bonds in edits.items():
                        for bond in bonds:
                            if edit_type != 'A':
                                rxn_labels.append(_label_tuple('r', real_sites.index(bond), template_dicts['real']['%s_%s' % (template_H, edit_type)]))
                            else:
                                rxn_labels.append(_label_tuple('v', virtual_sites.index(bond), template_dicts['virtual']['%s_%s' % (template_H, edit_type)]))
                    reactants.append(reactant)
...
def make_simulate_output(args, split = 'test'):
    ...
    with open('%s/simulate_output.txt' % args['output_dir'], 'w') as f:
        ...
        for i in df.index:
            labels = []
            for y in eval(df['Labels'][i], {'np': np}):
                if y != 0:
                    labels.append(y)
            if len(labels) == 0:
                labels = [(0, 0)]
