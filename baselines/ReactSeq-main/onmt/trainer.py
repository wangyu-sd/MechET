...
def build_trainer(opt, device_id, model, vocabs, optim, model_saver=None):
    ...
    trainer = Trainer(
        ...
        progress_bar=not getattr(opt, "disable_progress_bar", False),
    )
    return trainer
...
class Trainer(object):
    ...
    Args:
        ...
        progress_bar (bool): whether to display a live training progress bar.
        """
    ...
    def train(
        ...
        start_step = self.optim.training_step
        progress = tqdm(
            total=train_steps if train_steps > 0 else None,
            initial=max(0, start_step - 1),
            desc="Training",
            unit="step",
            dynamic_ncols=True,
            mininterval=0.5,
            disable=not (self.progress_bar and self.gpu_rank <= 0),
        )
            ...
            for i, (batches, normalization) in enumerate(
                ...
                if report_stats.n_words > 0:
                    progress.set_postfix(
                        loss=f"{report_stats.xent():.4f}",
                        ppl=f"{report_stats.ppl():.2f}",
                        acc=f"{report_stats.accuracy():.1f}%",
                        lr=f"{self.optim.learning_rate():.3g}",
                        refresh=False,
                    )
                progress.update(1)
                ...
                if valid_iter is not None and step % valid_steps == 0:
                    progress.set_description("Training (validating)", refresh=True)
                    valid_stats = self.validate(
                        valid_iter, moving_average=self.moving_average
                    )
                    progress.set_description("Training", refresh=True)
