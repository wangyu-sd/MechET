...
class RetroBridgeDataset(InMemoryDataset):
    ...
    def __init__(self, stage, root, extra_nodes=False, swap=False):
        ...
        self.strict = bool(self.metadata.get('strict', False))
        if self.metadata:
            atom_decoder = self.metadata['atom_decoder']
            if not atom_decoder or atom_decoder[-1] != '*':
                raise ValueError("RetroBridge metadata atom_decoder must end with '*'")
            self.types = {atom: idx for idx, atom in enumerate(atom_decoder)}
            self.max_n_dummy_nodes = int(self.metadata['max_n_dummy_nodes'])
        else:
            self.types = dict(type(self).types)
            self.max_n_dummy_nodes = RetroBridgeDatasetInfos.max_n_dummy_nodes
        ...
        if swap:
            self.data = Data(
                ...
                y=self.data.y, idx=self.data.idx, stable_id=self.data.stable_id,
                source_row_idx=self.data.source_row_idx,
                r_smiles=self.data.p_smiles, p_smiles=self.data.r_smiles,
            )
            self.slices = {
                'x': self.slices['p_x'],
                ...
                'y': self.slices['y'],
                'idx': self.slices['idx'],
                'stable_id': self.slices['stable_id'],
                'source_row_idx': self.slices['source_row_idx'],
                'p_x': self.slices['x'],
    ...
    def process(self):
        ...
        for i, row in tqdm(table.iterrows(), total=len(table)):
            ...
            stable_id = str(row['id']) if 'id' in table.columns else str(i)
            ...
            if rmol is None or pmol is None:
                failures.append((stable_id, 'RDKit failed to parse the reaction pair'))
                continue
            r_num_nodes = rmol.GetNumAtoms()
            p_num_nodes = pmol.GetNumAtoms()
            if p_num_nodes > r_num_nodes:
                failures.append((stable_id, f'product atoms {p_num_nodes} > precursor atoms {r_num_nodes}'))
                continue

            if self.extra_nodes:
                new_r_num_nodes = p_num_nodes + self.max_n_dummy_nodes
                if r_num_nodes > new_r_num_nodes:
                    message = f'|r|-|p| exceeds max_n_dummy_nodes: r={r_num_nodes}, p={p_num_nodes}'
                    if self.strict:
                        failures.append((stable_id, message))
                        continue
                    print(f'Molecule with {message}')
                    if self.stage == 'test':
            ...
            try:
                ...
                failures.append((stable_id, str(e)))
                continue
            ...
            if len(r_x) == len(p_x) and not torch.allclose(r_x[product_mask], p_x[product_mask]):
                failures.append((stable_id, 'incorrect atom mapping'))
                continue

            if self.stage == 'train' and len(p_edge_attr) == 0:
                if self.strict:
                    failures.append((stable_id, 'product graph has no bonds'))
                    continue
                else:
                    continue
            ...
            data = Data(
                ...
                stable_id=stable_id, source_row_idx=i,
        ...
        if failures and self.strict:
            preview = '; '.join(f'{stable_id}: {reason}' for stable_id, reason in failures[:10])
            raise RuntimeError(f'Failed to process {len(failures)} reactions. First failures: {preview}')
...
class RetroBridgeDataModule(MolecularDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        self.atom_encoder = dict(datasets['train'].types)
        self.atom_decoder = [
            atom for atom, _ in sorted(self.atom_encoder.items(), key=lambda item: item[1])
        ]
        self.max_n_dummy_nodes = datasets['train'].max_n_dummy_nodes
        self.metadata = datasets['train'].metadata
...
class RetroBridgeDatasetInfos:
    ...
    def __init__(self, datamodule):
        self.atom_encoder = dict(getattr(datamodule, 'atom_encoder', type(self).atom_encoder))
        self.atom_decoder = list(getattr(datamodule, 'atom_decoder', type(self).atom_decoder))
        self.max_n_dummy_nodes = int(
            getattr(datamodule, 'max_n_dummy_nodes', type(self).max_n_dummy_nodes)
        )
        ...
        self.possible_num_dummy_nodes = list(range(self.max_n_dummy_nodes + 1))
