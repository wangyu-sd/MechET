...
class MolecularVisualization:
    ...
    def visualize(self, path: str, molecules: list, num_molecules_to_visualize: int, log='graph', prefix='', suffix=''):
        ...
        for i in range(num_molecules_to_visualize):
            ...
            try:
                ...
                run = active_swanlab_run()
                if run is not None and log is not None:
                    print(f"Saving {file_path} to SwanLab")
                    run.log({log: swanlab.Image(file_path)})
            except rdkit.Chem.KekulizeException:
                print("Can't kekulize molecule")
            except ValueError:
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix, trainer=None):
        ...
        run = active_swanlab_run()
        if run is not None:
            print(f"Saving {gif_path} to SwanLab")
            run.log({"chain": swanlab.Video(gif_path)})
