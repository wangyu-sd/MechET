...
from src.metrics.molecular_metrics_discrete import (
    DummyTrainMolecularMetricsDiscrete,
    TrainMolecularMetricsDiscrete,
)
from src.metrics.sampling_metrics import DummySamplingMolecularMetrics, SamplingMolecularMetrics
...
from src.loggers.swanlab_logger import SwanLabLogger
...
def main(args):
    ...
    if datamodule.metadata:
        # The published detailed per-element metric classes only cover the
        # USPTO-50K vocabulary.  They are diagnostics, not the bridge loss.
    ...
    if args.disable_swanlab:
        ...
        run_logger = [csv_logger, SwanLabLogger(
            project=getattr(args, 'swanlab_project', 'RetroBridge'),
            workspace=getattr(args, 'swanlab_workspace', None),
            mode=getattr(args, 'swanlab_mode', 'online'),
            save_dir=args.logs,
            log_dir=os.path.join(args.logs, 'swanlab'),
            experiment_name=experiment,
            run_id=experiment,
            resume='must' if args.resume is not None else 'allow',
            group=args.dataset,
            job_type='train',
            tags=['RetroBridge', args.dataset, args.model],
        )]
    trainer = Trainer(
