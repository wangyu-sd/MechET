...
runtime_target=$(mktemp -d /tmp/mechet_action_delta_runtime.XXXXXX)
...
required = [Path(cfg["train_file"]), Path(cfg["validation_file"]), manifest_path]
missing = [str(path) for path in required if not path.is_file()]
if missing:
    raise SystemExit(f"missing action-delta artifacts: {missing}")
manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
if manifest.get("observation_mode") != "action_delta_v1":
    raise SystemExit(f"manifest is not action-only: {manifest_path}")
if manifest.get("intermediate_state_model_visible") is not False:
    raise SystemExit("manifest permits intermediate model-visible state")
for split, key in (
...
exec torchrun --standalone --nproc_per_node=8 \
  scripts/train_tool_sft.py --config "$training_config"
