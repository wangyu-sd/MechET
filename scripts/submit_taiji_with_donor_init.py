...
def validate_submission_policy(config: dict[str, object], config_path: Path) -> None:
    ...
    if not task_flag.startswith(REQUIRED_TASK_PREFIX):
        raise ValueError(
            f"Taiji task_flag must start with {REQUIRED_TASK_PREFIX!r}: {config_path}"
        )
    if not readable_name.lower().startswith(REQUIRED_TASK_PREFIX):
        raise ValueError(
            f"Taiji readable_name must start with {REQUIRED_TASK_PREFIX!r}: {config_path}"
        )
    if REQUIRED_LOG_WRAPPER not in start_cmd:
        raise ValueError(
            f"Taiji start_cmd must use {REQUIRED_LOG_WRAPPER}: {config_path}"
        )
    if re.search(r"(?:^|[;&|\s])(?:[12]?>>?|&>)", start_cmd):
        raise ValueError(
            f"Taiji start_cmd must not redirect stdout/stderr: {config_path}"
        )
