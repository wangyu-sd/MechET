...
local_output=$shared_repo/outputs/eval/natural_language_event_valid256_final_k1_a100_20260914
suffix_output=$shared_repo/outputs/eval/natural_language_event_suffix_valid32_final_k1_a100_20260914
...
runtime_target=$(mktemp -d /tmp/mechet_nl_final_smoke.XXXXXX)
...
echo "[natural-language-final-smoke] phase=gold-state-local valid_reactions=256"
torchrun --standalone --nproc_per_node=8 \
  ...
  --batch-size 2 --max-new-tokens 512 --max-context 4096
python -u "$runtime_repo/scripts/eval_natural_language_event_local.py" aggregate \
  --data "$data" --output "$local_output" --model Qwen/Qwen3-8B \
  --adapter "$adapter" --sample-reactions 256 --seed 17

echo "[natural-language-final-smoke] phase=closed-loop-suffix reactions=32 horizons=1,2,3,full"
torchrun --standalone --nproc_per_node=8 \
  ...
  --horizons 1 2 3 full --history-window 6 \
...
echo "[natural-language-final-smoke] complete local=$local_output/evaluation.json suffix=$suffix_output/suffix_evaluation.json"
