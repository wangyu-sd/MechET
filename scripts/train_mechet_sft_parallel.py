...
_WORKER_MAX_SEQ_LENGTH = 1024
_WORKER_TRAIN_FILE = "<train-file>"


def _encode_worker_init(model_path: str, max_seq_length: int, train_file: str) -> None:
    from transformers import AutoTokenizer

    global _WORKER_TOKENIZER
    global _WORKER_MAX_SEQ_LENGTH
    global _WORKER_TRAIN_FILE

    _WORKER_MAX_SEQ_LENGTH = max_seq_length
    _WORKER_TRAIN_FILE = train_file
    _WORKER_TOKENIZER = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True, local_files_only=True)
    if _WORKER_TOKENIZER.pad_token is None:
        _WORKER_TOKENIZER.pad_token = _WORKER_TOKENIZER.eos_token


def _encode_rows_chunk(chunk):
    if not chunk:
        return []
    ...
    for row_index, row in chunk:
        try:
            encoded = encode_assistant_only(_WORKER_TOKENIZER, row["messages"], max_length=_WORKER_MAX_SEQ_LENGTH)
        except Exception as exc:
            raise ValueError(f"{_WORKER_TRAIN_FILE}:{row_index + 1}: failed to encode assistant-only labels: {exc}") from exc

        label_count = sum(1 for value in encoded["labels"] if value != -100)
        encoded_pairs.append((row_index, encoded, label_count))
    return encoded_pairs


def _chunk_rows(rows, chunk_size: int):
    if not rows:
        return []
    if chunk_size <= 0:
...
def main() -> int:
    ...
    parser.add_argument("--num-encode-processes", type=int, default=4)
    ...
    num_encode_processes = max(1, int(_cfg("num_encode_processes", 4)))
    ...
    num_encode_processes = min(num_encode_processes, available_cpus, len(rows))
    encode_chunk_size = 64

    encoded_rows = []
    label_token_counts = []
    indexed_rows = list(enumerate(rows))
    row_chunks = _chunk_rows(indexed_rows, chunk_size=encode_chunk_size)

    chunk_sizes = [len(chunk) for chunk in row_chunks]
    print(
        f"Encoding rows with {num_encode_processes} processes, chunk_size={encode_chunk_size}, "
        f"total_chunks={len(row_chunks)}, chunk_sizes(first_20)={chunk_sizes[:20] if len(chunk_sizes) > 20 else chunk_sizes}"
    )
    with get_context("spawn").Pool(
        processes=min(num_encode_processes, len(row_chunks)),
        initializer=_encode_worker_init,
        initargs=(str(model_path), max_seq_length, str(train_file)),
    ) as pool:
        encoded_chunks = []
        with tqdm(total=len(indexed_rows), desc="Encoding rows", unit="row") as progress:
            for chunk_result in pool.imap_unordered(_encode_rows_chunk, row_chunks):
                encoded_chunks.append(chunk_result)
                progress.update(len(chunk_result))

    encoded_pairs = [item for chunk in encoded_chunks for item in chunk]
    encoded_pairs.sort(key=lambda item: item[0])
    encoded_rows = [item[1] for item in encoded_pairs]
    label_token_counts = [item[2] for item in encoded_pairs]
