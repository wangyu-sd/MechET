...
import bitsandbytes,importlib.metadata,rdkit,torch
...
if importlib.metadata.version('bitsandbytes') != '0.49.2':
    raise SystemExit(f"wrong bitsandbytes runtime: {bitsandbytes.__version__}")
...
print({'runtime_gate':'passed','rdkit':rdkit.__version__,'bitsandbytes':bitsandbytes.__version__,'gpus':names},flush=True)
PY
